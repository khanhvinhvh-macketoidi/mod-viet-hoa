...
export async function GET(){const auth=await admin();if(auth.response)return auth.response;try{const state=await readState();return NextResponse.json({ok:true,completed:Boolean(state),state})}catch(error){console.error(error);return NextResponse.json({ok:false,message:'Không thể đọc trạng thái chuyển đổi.'},{status:500})}}
export async function POST(){const auth=await admin();if(auth.response||!auth.user)return auth.response;try{const existing=await readState();if(existing)return NextResponse.json({ok:false,completed:true,message:'Quá trình chuyển đổi này đã được thực hiện trước đó.',state:existing},{status:409});const result=await migrateUsers();const state:State={migration:'user-profile-v1',completedAt:new Date().toISOString(),completedByUserId:auth.user.id,result};await writeState(state);return NextResponse.json({ok:true,completed:true,message:'Chuyển đổi dữ liệu Đạo Tịch đã hoàn tất.',state})}catch(error){console.error(error);return NextResponse.json({ok:false,message:error instanceof Error?error.message:'Chuyển đổi thất bại.'},{status:500})}}
